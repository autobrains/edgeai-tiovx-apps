...
int32_t aewb_logger_read_log_from_buffer(int8_t * src_buffer, int32_t buffer_size, AewbHandle *handle, tivx_h3a_data_t *h3a_ptr, tivx_ae_awb_params_t *aewb_ptr) {
    ...
    for (int i=0; i<sizeof(fields)/sizeof(field_t); i+=1) {
        ...
        field_t *f = &fields[i];
        if (num_bytes_read + f->size > buffer_size) {
            printf("aewb_logger_read_log_to_buffer: buffer smaller than expected, aborting write, i=%d, buffer_size=%d\n", i, buffer_size);
            break;
        }

        // printf("i=%d\n", i);
        memcpy(f->addr, buf_ptr, f->size);
        buf_ptr += f->size;
        num_bytes_read += f->size;
    }
...
int32_t aewb_logger_recv_bytes(aewb_logger_receiver_state_t *p_state)
{
    ...
    p_state->num_bytes_written = recvfrom(p_state->sock_fd, (int8_t*)p_state->buffer, sizeof(p_state->buffer),
                 MSG_WAITALL, (struct sockaddr *)&src_addr,
                 &src_addr_len);

    return p_state->num_bytes_written;
}
...
int32_t aewb_logger_recv_log(aewb_logger_receiver_state_t *p_state, AewbHandle *handle, tivx_h3a_data_t *h3a_ptr, tivx_ae_awb_params_t *aewb_ptr)
{
    ...
    aewb_logger_recv_bytes(p_state);

    return aewb_logger_read_log_from_buffer(p_state->buffer, p_state->num_bytes_written, handle, h3a_ptr, aewb_ptr);
}
